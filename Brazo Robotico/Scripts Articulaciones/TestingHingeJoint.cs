...
public class TestingHingeJoint : MonoBehaviour
{
    void Update()
    {
        ...
        motor.force = 1000; // Fuerza de la bisagra contra la gravedad u otros objetos

        if(Input.GetKey(KeyCode.A)){  //Si la tecla A se pulsa, targetVelocity tendra un valor de -90
            motor.targetVelocity = -90;
        }
        else{
            if(Input.GetKey(KeyCode.D)){ //Si la tecla D se pulsa, targetVelocity tendra un valor de 90
                motor.targetVelocity = 90;
            }
        ...
        hinge.useMotor = true;   //Indica que se va a utilizar el motor
    }
}
